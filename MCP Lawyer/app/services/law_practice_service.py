...
class LawPracticeService:
    ...
    async def analyze_legal_issue(self, issue_description: str, jurisdiction: str, practice_area: str) -> str:
        ...
        """
        system_prompt = f"""You are a legal analysis assistant for a Canadian law firm.
        Provide a thorough analysis of legal issues based on Canadian law in {jurisdiction}.
        Focus on the practice area of {practice_area}.
        Include relevant statutes, case law, and practical guidance.
        ...
        Provide an analysis of the legal principles, relevant statutes and case law, and practical guidance.
        """
        
        analysis = await self.ai_processor.generate_response(system_prompt, user_prompt)
    ...
    async def analyze_client_intake(self, practice_area: str, form_data: Dict[str, str]) -> Dict[str, Any]:
        ...
        """
        # Verify the practice area exists
        if practice_area not in self.client_intake_forms:
            raise HTTPException(status_code=404, detail=f"Client intake form for practice area '{practice_area}' not found")
        
        # Get the form definition to understand the fields
        form_definition = self.client_intake_forms[practice_area]
